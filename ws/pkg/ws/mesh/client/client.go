...
func New(ctx context.Context, cfg Config) (*MeshClient, error) {
	address, err := getAddress(ctx, cfg.ServiceName, cfg.TargetName)
	if err != nil {
	...
	if clientCfg.TLS != nil && clientCfg.TLS.ExpectedPeerID == "" {
		clientCfg.TLS.ExpectedPeerID = cfg.TargetName
	}
...
func getAddress(
	...
	address = strings.ReplaceAll(address, "-sidecar", "")
	address = strings.ReplaceAll(address, "http", "ws")
