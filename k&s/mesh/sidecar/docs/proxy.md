...
Sidecar прозрачно перехватывает TCP-трафик приложения и маршрутизирует его через listener-порты. Документ задает контракт перехвата и mTLS-маршрутизации для MVP.
...
`inboundMTLSPort` используется только при `mtlsEnabled=true`. Для режима без mTLS используйте `mtlsEnabled: false` и `inboundMTLSPort: 0`.
...
Этот режим применяется только при `mtlsEnabled=true`.
